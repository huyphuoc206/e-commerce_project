...
public class CommentDAO extends AbstractDAO<CommentDAO> implements ICommentDAO {
    ...
    public List<CommentModel> findOneByStatus(int status) {
        String sql = "SELECT c.* , u.fullname, u.avatar, p.name FROM comment c , user u, product p WHERE c.userid = u.id AND c.productid = p.id AND c.status = ?";
        return query(sql, new CommentMapper(), status);
    }
